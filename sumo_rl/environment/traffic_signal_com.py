...
class TrafficSignal:
    ...
    def get_connected_junction(self, lane_id, current_junction_id):
        # links = self.sumo.lane.getLinks(lane_id)
        
        # for link in links:
        #     connected_lane_id = link[0]
        #     direction = link[1]
        #     neighbor_lane_id = link[2]
        #     print("c: ", connected_lane_id, " d: ", direction, " n: ", neighbor_lane_id, " l: ", lane_id, " j: ", current_junction_id)

        #     if neighbor_lane_id == lane_id:
        #         #current lane is the successor of the connected lane
        #         connected_junction_id = self.sumo.lane.getJunction(connected_lane_id)
        #         return connected_junction_id
        ...
        if edge_id:
            ...
            to_node = net.getEdge(edge_id).getToNode()
            if to_node.getType() == "traffic_light":
                to_node_id = to_node.getID()
            else:
                return None
            #from_node = net.getEdge(edge_id).get
            #print("t: ", to_node_id, " c: ", current_junction_id)
            #connected_junction_id = to_node.getID()
            #return connected_junction_id
            return to_node_id
        else:
            # Handle the case where the lane is not part of any edge
            return None
    ...
    def get_neighbors(self):
        ...
        for lane in self.out_lanes:
            ...
            neighbor = self.get_connected_junction(lane, self.id)
            #print("neighbor: ", neighbor)
            if neighbor is not None and neighbor not in neighbors:
                neighbors.append(neighbor)
        #print("Junction ", self.id, " has neighbors ", neighbors)
        return neighbors
